...
class AutomationModify extends StatefulWidget {
  const AutomationModify({Key? key}) : super(key: key);

  @override
  State<AutomationModify> createState() => _AutomationModifyState();
}

class _AutomationModifyState extends State<AutomationModify> {
  ...
  void initState() {
    ...
    appClass.users[userIdentifier].universes[universeIdentifier].automations[automationIdentifier].preconditions.isEmpty
        ? {
            ...
            preconditionsSwitch = false
          }
        : {automationPreconditions = appClass.users[userIdentifier].universes[universeIdentifier].automations[automationIdentifier].preconditions.first, preconditionsSwitch = true};
    super.initState();
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        title: Text(modifyAutomationTitleMessageTextLanguageArray[languageArrayIdentifier]),
        centerTitle: true,
      ),
                  ...
                  if (myAutomationName.text.isNotEmpty) {
                    if (automationActions.isNotEmpty) {
                      if (automationActions.last['action_executor'] != 'delay') {
                        await appClass.users[userIdentifier].universes[universeIdentifier].automations[automationIdentifier].modifyAutomation(
