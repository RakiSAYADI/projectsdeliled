...
static esp_err_t event_handler(void *ctx, system_event_t *event)
{
    ...
    case SYSTEM_EVENT_STA_GOT_IP:
        ...
        ESP_LOGI(TAG,"IP: %s\r",ip4addr_ntoa(&event->event_info.got_ip.ip_info.ip));
        ESP_LOGI(TAG,"MASK: %s\r",ip4addr_ntoa(&event->event_info.got_ip.ip_info.netmask));
        ESP_LOGI(TAG,"GATEWAY: %s\r",ip4addr_ntoa(&event->event_info.got_ip.ip_info.gw));

		sprintf(UnitCfg.Static_IP.IP,ip4addr_ntoa(&event->event_info.got_ip.ip_info.ip));
		sprintf(UnitCfg.Static_IP.MASK,ip4addr_ntoa(&event->event_info.got_ip.ip_info.netmask));
		sprintf(UnitCfg.Static_IP.GATE_WAY,ip4addr_ntoa(&event->event_info.got_ip.ip_info.gw));
...
void wifi_init_sta()
{
    ...
    // Start SNTP Task
    xTaskCreatePinnedToCore(&sntp_task, "sntp_task", 4000, NULL, 1, NULL,1);
    // Start FTP Task
    xTaskCreatePinnedToCore(&ftp_task, "ftp_task", 4000, NULL, 1, NULL,1);
    // Start MQTT Client Task
    xTaskCreatePinnedToCore(&mqtt_app_start, "mqtt_app_start", 4000, NULL,1, NULL,1);
    // Start UDP CLIENT Service
    xTaskCreatePinnedToCore(&udp_app_start, "udp_app_start",5000 , NULL, 1, NULL,1);
    ...
    // Start SSL CLIENT Service
    //xTaskCreatePinnedToCore(&ssl_client, "ssl_client",16000 , NULL, 5, NULL,1);
