...
double GetTemp(float AdcValue)
{
	...
	AdcT = ((8.194-sqrt((8.194*8.184)+4*0.00262*(1324-AdcValue)))/(2*(-0.00262)))+30;
	return(AdcT);
}
...
void adc_task()
{
    ...
    while (1)
    {
        ...
        //Multisampling
        for (int i = 0; i < UnitCfg.SAMPLES; i++)
        {
        ...
        adc_reading_vtana /= UnitCfg.SAMPLES;

        PirdeltaRMS = sqrt(PirdeltaSum/UnitCfg.SAMPLES);

        if (PirdeltaRMS>UnitCfg.PirSensitivity) Occur++;
        if (Occur>1)
		{
			PirFlag=1;
			Occur=0;
			/*gettimeofday(&tv_adc, NULL);
			localtime_r(&tv_adc.tv_sec, &now_adc);
			UnitData.LastDetTime=tv_adc.tv_sec;
			strftime(strftime_buf, sizeof(strftime_buf), "%c", &now_adc);
			printf("The current date/time in Timezone is: %s\n",strftime_buf);*/
			time(&UnitData.LastDetTime);
		    //printf("The current date/time is: %ld\n",UnitData.LastDetTime);
		    if(UnitCfg.Summer_time)
		    {
		    	UnitData.LastDetTime-=3600;
		    }
