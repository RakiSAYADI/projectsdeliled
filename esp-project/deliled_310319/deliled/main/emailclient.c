...
void payloadBuild()
{
    ...
    sprintf(payload_text[4],"Subject: Alerte niveau de CO2 [%s] \r\n",UnitCfg.UnitName);
    sprintf(payload_text[5],"\r\n");// empty line to divide headers from body, see RFC5322
    sprintf(payload_text[6],"Votre boitier [%s] vous informe qu’un taux de CO2 élevé a été détecté. \r\n",UnitCfg.UnitName);
    sprintf(payload_text[7],"La valeur de CO2 relevée est la suivante : %d \r\n",UnitData.aq_Co2Level);
    sprintf(payload_text[8],"\r\n");
    sprintf(payload_text[9],"Il est préférable d’aérer les zones dont le niveau de CO2 est trop élevé. \r\n");
