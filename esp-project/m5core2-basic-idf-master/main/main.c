...
#define TEST_BEFORE_GUI_START false
// If true, we'll bring up the GUI
#define START_GUI true
...
static void gui_thread(void *pvParameter)
{
	...
	lv_cont_set_layout(root, LV_LAYOUT_ROW_MID);
	...
	for (int i = 0; i < sizeof(switches) / sizeof(switches[0]); i++)
	{
		...
		lv_style_init(&style_swt);
		lv_style_set_bg_color(&style_swt, LV_STATE_DEFAULT, LV_COLOR_GREEN);
		lv_obj_add_style(toggle, LV_OBJ_PART_MAIN, &style_swt);
