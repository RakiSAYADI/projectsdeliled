...
void LedStatusTask()
{
	...
	while (1)
	{
		...
		if (((gpio_get_level(GPIO_NUM_16) == 1) || (gpio_get_level(GPIO_NUM_4) == 1)) && (UVTreatementIsOn))
		{
...
void LedStatInit()
{
	...
	//Led and relay task
	xTaskCreate(&LedStatusTask, "LedStatusTask", configMINIMAL_STACK_SIZE * 3, NULL, 1, NULL);
}
