...
bool stopTimeTrimBattery = false;

uint16_t bat_t = 0;
uint16_t esp_t = 0;
time_t i2c_now = 0;
struct timeval tv;
struct tm esp_timeinfo = {0};
struct tm i2c_timeinfo = {0};
struct tm *i2c_timeinfo_nogmt;
char strftime_buf[64];
char strftimebattery_buf[64];

int lostSeconds = 0;
uint16_t durationTime = 0;
#define CHOSENPERIOD 60
...
esp_err_t i2c_write_MCP7940(i2c_port_t i2c_num, uint8_t reg, MCP7940_Time_Typedef *time)
{
	...
	case MCP7940_CON_CONFIG:
		...
		data = data + (uint8_t)(time->externalOsc << 3);
		...
		break;
	case MCP7940_OSC_CONFIG:
		data = (uint8_t)(time->trimVal);
		data = data + (uint8_t)(time->sign << 7);
		break;
	}
	i2c_master_write_byte(cmd, reg, ACK_CHECK_EN);
...
esp_err_t saveTimeInsideBattery(struct tm timeinfo)
{
	...
	ESP_ERROR_CHECK(ret);
	ret = i2c_write_MCP7940(I2C_MASTER_NUM, MCP7940_CON_CONFIG, &time_MCP7940);
	ESP_ERROR_CHECK(ret);
...
void i2c_test_task(void *arg)
{
	...
	while (1)
	{
		...
		if (clockSavorEnabled)
		{
			...
			ret = i2c_read_MCP7940(I2C_MASTER_NUM, MCP7940_OSC_CONFIG, &time_MCP7940);
			if (ret == ESP_ERR_TIMEOUT)
			{
				ESP_LOGE(TAG, "I2C MCP7940 timeout\n");
			}
			else if (ret == ESP_OK)
			{
				...
				else
				{
					...
					{
						esp_timeinfo.tm_year = time_MCP7940.year;
						esp_timeinfo.tm_mon = time_MCP7940.month;
						esp_timeinfo.tm_mday = time_MCP7940.day;
						esp_timeinfo.tm_wday = time_MCP7940.day_of_week;
						esp_timeinfo.tm_hour = time_MCP7940.hour;
						esp_timeinfo.tm_min = time_MCP7940.minute;
						esp_timeinfo.tm_sec = time_MCP7940.second;
						bat_t = ((esp_timeinfo.tm_min * 60) + esp_timeinfo.tm_sec);
						esp_t = ((i2c_timeinfo.tm_min * 60) + i2c_timeinfo.tm_sec);
						lostSeconds = abs(bat_t - esp_t);
						// ESP_LOGI(TAG, "I2C MASTER Checking time difference %d", lostSeconds);
						/*ESP_LOGI(TAG, "I2C MASTER Read TIMER( MCP7940 ) : %d:%d:%d:%d:%d:%d",
								 time_MCP7940.squareWaveEnabled, time_MCP7940.alarmOneEnabled, time_MCP7940.alarmTwoEnabled,
								 time_MCP7940.externalOsc, time_MCP7940.coarseTrimEnabled, time_MCP7940.squareWaveSelect);*/
						durationTime++;
						// ESP_LOGI(TAG, "I2C MASTER trim val %d", (uint8_t)((((lostSeconds * 1000000) / 360) * 32768 * 60) / 2000000));
						if (lostSeconds <= 1 && stopTimeTrimBattery)
						{
							time_MCP7940.trimVal = 0;
							time_MCP7940.coarseTrimEnabled = 0;
							ret = i2c_write_MCP7940(I2C_MASTER_NUM, MCP7940_OSC_CONFIG, &time_MCP7940);
							ESP_ERROR_CHECK(ret);
							stopTimeTrimBattery = false;
						}
						if (durationTime == CHOSENPERIOD)
						{
							if (lostSeconds > 1 && lostSeconds < 3000)
							{
								if (bat_t < esp_t)
								{
									time_MCP7940.sign = 1;
								...
								time_MCP7940.trimVal = 0;
								time_MCP7940.coarseTrimEnabled = 0;
								time_MCP7940.squareWaveSelect = 0;
								ret = i2c_write_MCP7940(I2C_MASTER_NUM, MCP7940_CON_CONFIG, &time_MCP7940);
								ret = i2c_write_MCP7940(I2C_MASTER_NUM, MCP7940_OSC_CONFIG, &time_MCP7940);
								ESP_ERROR_CHECK(ret);
								time_MCP7940.coarseTrimEnabled = 1;
								time_MCP7940.squareWaveSelect = 3;
								time_MCP7940.trimVal = (uint8_t)((((lostSeconds * 1000000) / CHOSENPERIOD) * 32768 * 60) / 2000000);
								if (time_MCP7940.trimVal >= 128)
								{
									time_MCP7940.trimVal = 127;
								}
								ret = i2c_write_MCP7940(I2C_MASTER_NUM, MCP7940_CON_CONFIG, &time_MCP7940);
								ret = i2c_write_MCP7940(I2C_MASTER_NUM, MCP7940_OSC_CONFIG, &time_MCP7940);
								ESP_LOGI(TAG, "I2C MASTER Sending the TRIM VALUE %d", time_MCP7940.trimVal);
								ESP_ERROR_CHECK(ret);
								stopTimeTrimBattery = true;
							}
							durationTime = 0;
						}
					}
				...
				strftime(strftimebattery_buf, sizeof(strftimebattery_buf), "%c", &esp_timeinfo);
				ESP_LOGI(TAG, "I2C MASTER Read TIMER( MCP7940 ) : %s, %d bat %d esp %d",
						 strftimebattery_buf, time_MCP7940.trimVal, bat_t, esp_t);
