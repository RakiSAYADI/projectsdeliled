...
#define SENDER_MAIL "delitech.alert@gmail.com"
#define SENDER_PASSWORD "maestro_user"

#define RECIPIENT_MAIL "raki_sayadi@hotmail.fr"

#define SERVER_USES_STARTSSL true
...
void smtp_client_task()
{
    while (true)
    {
        if (AP_CONNECT && sendEmail)
        {
            ...
            ret = mbedtls_base64_encode((unsigned char *)base64_buffer, sizeof(base64_buffer), &base64_len, (unsigned char *)SENDER_MAIL, strlen(SENDER_MAIL));
            if (ret != 0)
            {
            ...
            ret = mbedtls_base64_encode((unsigned char *)base64_buffer, sizeof(base64_buffer), &base64_len, (unsigned char *)SENDER_PASSWORD, strlen(SENDER_PASSWORD));
            if (ret != 0)
            {
                ESP_LOGE(TAG, "Error in mbedtls encode! ret = -0x%x", -ret);
                goto exit;
            ...
            while (offset < esp_logo_png_end - 1)
            {
                ...
                ret = mbedtls_base64_encode((unsigned char *)base64_buffer, sizeof(base64_buffer), &base64_len, (unsigned char *)offset, read_bytes);
                if (ret != 0)
                {
                    ESP_LOGE(TAG, "Error in mbedtls encode! ret = -0x%x", -ret);
                    goto exit;
...
void smtpClient()
{
    xTaskCreate(&smtp_client_task, "smtp_client_task", TASK_STACK_SIZE * 4, NULL, 5, NULL);
}