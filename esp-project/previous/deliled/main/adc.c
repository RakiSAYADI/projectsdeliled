...
static const adc_channel_t channel_temp = ADC1_GPIO33_CHANNEL;     //
...
void adc_task()
{
    ...
    while (1)
    {
        ...
        for (int i = 0; i < NO_OF_SAMPLES; i++)
        {
                ...
                adc_reading_temp += adc1_get_raw((adc1_channel_t)channel_temp);
        }

        adc_reading /= NO_OF_SAMPLES;
        adc_reading_vref /= NO_OF_SAMPLES;
        adc_reading_temp /= NO_OF_SAMPLES;

        Pirdelta=adc_reading-adc_reading_vref;

        if (abs(Pirdelta)>UnitCfg.PirSensitivity) Occur++;
        if (Occur>2)
        	{
        		PirFlag=1;
        		Occur=0;
        ...
        /* TANA */
        uint32_t temp=esp_adc_cal_raw_to_voltage(adc_reading_temp,adc_chars)*(1650.0/rvref);
        UnitData.Temp = GetTemp(temp);
        //printf("%d mv,%0.2f\r\n",temp,UnitData.Temp);
