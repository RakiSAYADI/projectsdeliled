...
void add2buffer()
{
	memcpy(BufferArray[TxBufferitemCount].bufferitem,TxBuffer,sizeof(TxBuffer));
	if (TxBufferitemCount<63) TxBufferitemCount++;
}
...
void MilightHandler(uint8_t cmd,uint8_t subcmd,uint8_t zonecode)
{
	...
	for (i=0;i<4;i++)
	{
		...
		case LCMD_SWITCH_ON_OFF :
			if (subcmd==0) {Mi_On(zone);}
			if (subcmd==1) {Mi_Off(zone);}
			break;
		case LCMD_SWITCH_ON :
			if (subcmd==0) {Mi_On(zone);}
			break;
		...
		case LCMD_MODE_SETTING :
			if (subcmd==0) {Mi_spddw(zone);}
			if (subcmd==1) {Mi_spdup(zone);}
			if (subcmd==2)
				{
					Mi_mode(zone,LastMode);
					if (LastMode<8) LastMode++; else LastMode=0;
				}
			break;
		case LCMD_PAIRING :
			if (subcmd==0) {Mi_Unpair(zone);}
			if (subcmd==1) {Mi_Pair(zone);}
			break;
		case LCMD_SET_WHITE :
			Mi_White(zone);
			break;
...
void lightControl_Init()
{
	...
	xTaskCreatePinnedToCore(&AutoLightStateMachine, "AutoLightStateMachine", 2048*2, NULL, 5, NULL,1);
