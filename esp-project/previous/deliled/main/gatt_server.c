...
void char_total1_read_handler(esp_gatts_cb_event_t event, esp_gatt_if_t gatts_if, esp_ble_gatts_cb_param_t *param) {
	...
	if (read_profile_count==0){
		...
		sprintf((char*)total,"{\"profile_id\":\"%s\",\"PROFILE_1\":{\"pname\":\"%s\",\"pdata\":[%ld,%ld,%ld,%ld,\"%s\"],\"lum\":[%d,%d,\"%s\",%d,%d,%d,%d,%d,%d],"
				"\"veille\":[%d,%ld,\"%s\"],\"cycle\":[%d,\"%s\",%ld,%ld,%d,%ld,%ld,%d,%ld,%ld,%d]},"
													"\"PROFILE_2\":{\"pname\":\"%s\",\"pdata\":[%ld,%ld,%ld,%ld,\"%s\"],\"lum\":[%d,%d,\"%s\",%d,%d,%d,%d,%d,%d],"
				"\"veille\":[%d,%ld,\"%s\"],\"cycle\":[%d,\"%s\",%ld,%ld,%d,%ld,%ld,%d,%ld,%ld,%d]}}",
...
void char_total3_read_handler(esp_gatts_cb_event_t event, esp_gatt_if_t gatts_if, esp_ble_gatts_cb_param_t *param) {
	...
	sprintf((char*)total1,"{\"couleur1\":[\"%s\",%d,%d,%d,%d,%d],"
						   ...
						   "\"co2\":[%d,%d,\"%s\",%d,\"%s\",\"%s\"],"
						   "\"modem\":\"%s\",\"IP_STATIC\":[%d,\"%s\",\"%s\",\"%s\",\"%s\"],\"mode\":\"%d\"}",
	...
	if (jsonparse(config_json,tmp,"co2",0)==0)
		{
			...
			if (UnitCfg.Co2LevelWarEnb) {ESP_LOGD(GATTS_TAG, "CO2 Warning Enabled");xTaskCreatePinnedToCore(&email_test_task, "email_test_task",16000 , NULL, 3, NULL,1);}
				else ESP_LOGD(GATTS_TAG, "CO2 Warning Disabled");
	...
	if (jsonparse(config_json,tmp,"co2",1)==0)
		{
			...
			ESP_LOGD(GATTS_TAG, "Co2 Level : %d",UnitCfg.Co2LevelWar);

			savenvsFlag = true;
		}
	if (jsonparse(config_json,UnitCfg.Co2LevelSelect,"co2",2)==0)
		{
			ESP_LOGD(GATTS_TAG, "Co2 Zone Selection : %s",UnitCfg.Co2LevelSelect);

			savenvsFlag = true;
		}
	if (jsonparse(config_json,UnitCfg.Email,"co2",3)==0)
		{

			ESP_LOGD(GATTS_TAG, "Co2 Alert Email : %s",UnitCfg.Email);
			savenvsFlag = true;
...
bool ProfileConfig()
{
			...
			if (jsonparse(config_json,tmp,"mode",0)==0)
				{
					...
					if (strcmp(tmp, "manu") == 0)
					{
						ESP_LOGI(GATTS_TAG, "Switch to manual mode");
						UnitCfg.state=UNIT_STAT_REG_MAN;
					...
					{
						ESP_LOGI(GATTS_TAG, "Switch to auto mode");
						UnitCfg.state=UNIT_STAT_REG_AUTO;
	...
	if (jsonparse(config_json,tmp,"pdata",1)==0)
		{
	...
	if (jsonparse(config_json,UnitCfg.UserLcProfile.Veille_days,"pdata",2)==0)
		{

		ESP_LOGI(GATTS_TAG, "User profile days set %s",UnitCfg.UserLcProfile.Veille_days);
		savenvsFlag = true;
	...
	if (jsonparse(config_json,tmp,"veille",0)==0)
		...
		UnitCfg.UserLcProfile.VeilleBrEnb = atoi(tmp);
		if (UnitCfg.UserLcProfile.VeilleBrEnb) ESP_LOGI(GATTS_TAG, "Standby brighness regulator Enabled");
			else ESP_LOGI(GATTS_TAG, "Standby brighness regulator Disabled");
		savenvsFlag = true;
	...
	if (jsonparse(config_json,tmp,"veille",1)==0)
		...
		tmpi=atoi(tmp);
		UnitCfg.UserLcProfile.NoticeTimeout = ((tmpi/100)*3600)+((tmpi%100)*60);
		ESP_LOGI(GATTS_TAG, "Notice timeout value %ld",UnitCfg.UserLcProfile.NoticeTimeout);
		savenvsFlag = true;

		}
	if (jsonparse(config_json,UnitCfg.UserLcProfile.veille_zone,"veille",2)==0)
		{

		ESP_LOGI(GATTS_TAG, "Standby zone set %s",UnitCfg.UserLcProfile.veille_zone);
	...
	if (jsonparse(light_json,tmp,"system",0)==0)
		{
			...
			if (sys_subcmd==0)
			{
				...
				UnitCfg.UserLcProfile.VeilleBrEnb = false;
